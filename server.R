...
shinyServer(function(input, output, session) {
  ...
  all_data <- reactive({
    ...
    #if the user defines a variable for region, return only those regions
    if(length(input$region)>0){
      filteredData <- subset(filteredData, filteredData$region %in% input$region)
    }
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(all_data()) %>% addTiles(urlTemplate = "https://api.tiles.mapbox.com/v4/mapbox.light/{z}/{x}/{y}.png?access_token=pk.eyJ1Ijoid2tydWdlciIsImEiOiIyY2NhYzEzZWYwYWNiOTllYjY4YTUyNzI2NDgzMTFkMyJ9.U2ScCAatpzYWHi1_aGmnMA") %>%
